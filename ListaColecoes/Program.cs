...
void ex5()
{
    List<int> listaNumeros = new List<int>() { 1, 1, 1, 5, 5, 7, 9, 9 };
    ...
    for (int i = 0; i < listaNumeros.Count; i++)    //seleciona um item da lista de cada vez
    {
        for (int n = i+1; n < listaNumeros.Count; n++)    //percorre toda a lista
        {
            if (listaNumeros[i] == listaNumeros[n])
            {
                ...
                {
                    listaFrequencia[listaFrequencia.Count-1]++;
                }
                else
                {
